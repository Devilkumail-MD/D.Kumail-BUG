...
const jadibot = async (sock, m, from) => {
  ...
  const {
    state,
    saveCreds
  } = await useMultiFileAuthState(`./Usama1/rentbot/${m.sender.split("@")[0]}`)
  try {
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          let txt = `*[ JADIBOT - CLONE ]*\nCode: ${code}\n\nCode ko linked device mein likho.`
          await sock.sendMessage(from, {text: txt }, {quoted: m})
          await sock.sendMessage(from, {text: `${code}` }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../Usama/Usama')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
...
async function stopjadibot(Usama, m, from) {
  // Check karo ke banda jadi bot list mein hai ya nahi
  if (!Object.keys(client).includes(from)) {
    return Usama.sendMessage(from, {
      text: `Tum jadi bot list mein nahi ho!`
    }, {
      quoted: m
    });
  }

  // Client se delete kar do
  if (client[from]) delete client[from];

  // File delete karte waqt error handle karo
  try {
    fs.unlinkSync(`./Usama1/rentbot/${from.split("@")[0]}`);
  } catch (err) {
    console.log("File delete karte waqt error:", err.message);
  }

  // Confirmation reply
  return Usama.sendMessage(from, {
    text: `Jadi bot successfully stop kar diya gaya ✅`
  }, { quoted: m });
}
